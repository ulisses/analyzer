...
void gravaGrafo(Grafo *grafo, char *nomeFile, ListaPontosInteresse *lpi)
{
	...
	for( ; lpi != NULL; lpi = lpi->prox)
	{
		...
		fprintf(fp, "%s:%s:%d:%d:%d:%d:%s:%d:%f:%f:%s\n", ponto->nome, showTipo(ponto->tipo), ponto->valorArquitectonico,
		        ...
		        ponto->distancia, ponto->tempoMedio, ponto->nome);
	}
/*	
...
Grafo *carregaGrafo(ListaRuas **lr, ListaPontosInteresse **lpi)
{
	FILE *fp = fopen("/home/ulisses/code/mp2/b.txt","r");
...
int main()
{
	...
	//gravaGrafo(g,"/home/ulisses/code/mp2/teste.txt",lpi);
/*	Rua *rua=NULL;
	Rua *rua1=NULL;
	ListaRuas *lr = NULL;
	Ponto * ponto = NULL;
